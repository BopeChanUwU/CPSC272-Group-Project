...
app.post('/', async (req, res) => {
    try {
        ...
        const sql = 'INSERT INTO public.users (id, user_name, first_name, last_name, email, password, imageUrl) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *;';
...
app.get('/', async (req, res) => {
    try {
        const sql = 'SELECT id, name, email FROM public.users ORDER BY id;';
...
app.put('/:id', async (req, res) => {
    try {
        ...
        const sql = 'UPDATE public.users SET name = $1, email = $2 WHERE id = $3 RETURNING *;';
...
app.read('/', async (req, res) => {
    try {
        ...
        const sql = 'SELECT id, user_name, first_name, last_name, email, password, imageUrl FROM public.users WHERE email=$1, password=$2;';
        const result = await db.query(sql, [id, user_name, first_name, last_name, email, password, imageUrl]);
